...
WHISPER_BUILD_DIR="${AMR_WHISPER_BUILD_DIR:-${WHISPER_CPP_DIR}/build}"
...
cmake -S "${WHISPER_CPP_DIR}" -B "${WHISPER_BUILD_DIR}" -DCMAKE_BUILD_TYPE=Release
cmake --build "${WHISPER_BUILD_DIR}" --target whisper-cli -j"$(nproc)"
...
Binary: ${WHISPER_BUILD_DIR}/bin/whisper-cli
