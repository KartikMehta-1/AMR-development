...
| 26 | Voice Command MVP | <span style="color: goldenrod">In Progress</span> | Steps 1-3 are implemented or in bring-up: text command parser, typed wake-word gating for "lovely", and laptop-mic Vosk ASR node. Continue in branch-sized phases: confirmation/rejection handling; Jetson speaker/TTS feedback; full voice loop; robustness guards. Use the existing mission runtime as the backend for `go home`, `go kitchen`, `go hall`, `stop`, and `status`; log transcripts, parsed intents, and mission outcomes. | 2026-04-30 | TBD |
