...
| 28 | Manipulation Perception + Calibration | <span style="color: goldenrod">In Progress</span> | Use AMR-mounted depth camera for broad scene/object discovery and wrist camera for close-range grasp refinement. Complete hand-eye calibration, base-to-arm extrinsics, camera-to-tool frames, AprilTag checks, RGB-D logging, and stale-frame rejection before grasp execution. | 2026-07-09 | TBD |
| 29 | Orin NX AMR Runtime Migration | <span style="color: goldenrod">In Progress</span> | Pulled forward as the pre-Week-27 focus while keeping NUC+Nano/Foxy as the validated fallback. Orin Docker profile created under `docker/orin`; image `amr/ros2-humble-orin:arm64` builds on Orin with ROS 2 Humble, Nav2, ros2_control, micro-ROS Agent, YDLidar driver, RealSense ROS driver, and librealsense `2.57.7` using RSUSB. Latest image checkpoint is `68c622991738`, rebuilt with the YDLidar Humble launch-file patch; `ros2 launch ydlidar_ros2_driver ydlidar_launch.py params_file:=...` now starts and publishes `/scan` at ~9.68 Hz. Initial hardware smoke found D455 on USB 3.2 and CP210x at `/dev/ttyUSB0`; RealSense ROS depth/color streams reached ~30 Hz in the container when run as root. A 120 s D455 topic-rate soak passed with depth ~29.77 Hz and color ~29.56 Hz, but USB access/log warnings still need cleanup before calling it reliability-cleared. YDLidar G4 connected through the stable CP2102 by-id path with `frame_id=lidar_link`; checksum/fixed-resolution warnings remain follow-ups. The old Nano-era STM firmware also connected through ST-LINK `/dev/ttyACM0`; micro-ROS Agent at 460800 exposed `/amr_stm/*`, wheel state at ~9.45 Hz, and fault mask `0` with no commands sent. AMR `ros_ws` now builds inside the Orin Humble image after porting `amr_hardware` to Humble `ros2_control` lifecycle callbacks and updating controller-manager spawners; added `ydlidar_orin.yaml` and `orin_hardware.launch.py` for stable by-id paths. Supervised suspended combined bring-up passed with both controllers active, `/scan` ~9.68 Hz, `/joint_states` present, `/amr_stm/wheel_state` ~9.79 Hz after STM reset, NUC RViz visibility over CycloneDDS, Orin dashboard/teleop helpers, Nav2 lifecycle on Humble, and RViz goal delivery after adding the Nav2 panel. A tiny suspended Nav2 CLI goal reached the STM wheel-command path and completed; do not use suspended testing for localization quality because wheel odom moves while the room scan does not. RealSense color is usable from RViz, but depth reliability remains suspect due USB/control-transfer and Right MIPI warnings, so depth stays diagnostic-only. Current pause: print/install the Orin enclosure, power the Orin from the AMR, then resume supervised floor bring-up and hardware acceptance. Do not treat Orin as the primary AMR computer until it passes supervised hardware acceptance. | 2026-06-06 | TBD |
...
| 34 | URDF/MoveIt For Base + SO-101 | <span style="color: goldenrod">In Progress</span> | Started the single mounted SO-101 scaffold: optional AMR Xacro integration, upstream SO101 URDF/STL import, AMR mount wrapper, wrist-camera TF frames, and planning-only named poses. Next: measure mount offsets, calibrate wrist-camera extrinsics, generate MoveIt2 config and limits, and verify planning scene in simulation/bench-safe mode. | 2026-06-25 | TBD |
...
- Current Focus (next sprint):
  - Print/install the Jetson Orin NX enclosure and power the Orin from the AMR before any further floor-motion testing.
  - Resume Orin floor bring-up only after the Orin is mechanically mounted, powered from the robot, and supervised with the robot on the floor.
  ...
  - Use the new Orin-specific Docker/runtime profile (`docker/orin`, image `amr/ros2-humble-orin:arm64`) as the Humble/JetPack 6 base. Next build the AMR ROS workspace there before attaching hardware.
  - Bring up Orin hardware incrementally: USB/device IDs, STM32 micro-ROS over USART2/ST-LINK VCOM, `/amr_stm/*` topics, LiDAR `/scan`, RealSense streams, optional Jetson-connected IMU, GPU/container runtime, thermals, network, and device permissions. Initial bench/suspended checks are complete enough to pause; the next meaningful validation is floor hardware acceptance.
...
- Next Focus:
  - Resume Orin NX AMR runtime migration after the enclosure/power integration is ready, while keeping NUC+Nano/Foxy as the known-good fallback until Orin passes hardware acceptance.
  ...
  - MoveIt-first manipulator bring-up after the AMR base is stable. Current first slice is the upstream-backed single SO-101 URDF/TF scaffold with wrist-webcam frame; driver/execution remains disabled.
