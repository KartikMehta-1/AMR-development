# Kartik's AMR Project Tracker (49 Weeks)
**File:** `AMR_project.md`  
**Owner:** Kartik Mehta  
**Last Updated:** 2026-07-07
**Scope:** STM32 low-level control, Jetson Nano/Orin NX high-level compute, motor drivers, current sensing (ACS758 x2), FreeRTOS, ROS2 + Gazebo, SLAM & Navigation; eventual goal is a fully autonomous AMR with one mounted SO-101 manipulator that can pick/place small objects using state-of-the-art VLA/VLM/LLM-based policies.
...
| 12 | Firmware v2: Differential Drive | <span style="color: green">Done</span> | Map (v, I%) -> wheel RPM; ramp/coordination added; saturation with curvature-preserving scaling; basic 5 s test sequence running. | 2026-01-29 | 2025-12-09 |
| 13 | Firmware v2: micro-ROS Bring-up | <span style="color: green">Done</span> | USART2 custom transport; `/amr_stm/wheel_cmd_left` and `/amr_stm/wheel_cmd_right` subs; `/amr_stm/wheel_state`, duty, current, ADC/zero, fault, safety, and `/amr_stm/ros_diag` pubs; `/amr_stm/enable`, `/amr_stm/estop`, `/amr_stm/clear_fault` wired; fault clear verified; fault mask documented; legacy UART telemetry disabled. | 2026-02-19 | 2026-01-04 |
| 14 | Dev PC Env & Tooling | <span style="color: green">Done</span> | Dockerized Foxy drivers + devpc image built (micro-ROS agent + YDLidar + RViz2/Gazebo); RViz2 config for `/scan` (best_effort QoS) working; ROS2 workspace build verified; Docker buildx workflow and Jetson builds validated; passwordless SSH to Jetson set up. | 2026-03-05 | 2026-01-18 |
| 15 | Jetson Nano ROS2/JetPack | <span style="color: green">Done</span> | Jetson container build complete; micro-ROS agent + teleop + YDLidar running; `/amr_stm/*` and `/scan` verified; depth camera running with `/camera/*` and `/points` validated; RViz2 visualization from dev PC confirmed; DDS interface bound to Wi-Fi; depth stream stabilized with lower profiles. | 2026-03-12 | 2026-01-24 |
| 16 | Wireless PC<->Nano | <span style="color: green">Done</span> | Wi-Fi adapter online and Jetson reachable over home network; passwordless SSH working; DHCP reservation set; ping avg ~10 ms and iperf ~15 Mbps verified; NTP sync active. Optional VPN (WireGuard/Tailscale) remains a nice-to-have. | 2026-03-19 | 2026-01-20 |
| 17 | URDF Modeling (Base AMR) | <span style="color: green">Done</span> | Base URDF/Xacro created with chassis, wheels, and sensor frames (LiDAR + depth cam); LiDAR/camera split into `lidar.xacro` + `camera.xacro`; D455 camera plugin added; base_footprint restored; wheel offsets corrected; caster clearance tuned; Gazebo launch defaults to `obstacles.world`; ros2_control config added; URDF validated in sim. | 2026-03-26 | 2026-02-11 |
| 18 | Navigation & Mapping Bring-up | <span style="color: green">Done</span> | Mapping: slam_toolbox (2D LiDAR) running on real LiDAR; map save/load + posegraph serialization verified. Localization: AMCL on saved map working. Navigation: Nav2 bring-up working with RViz goals; costmaps wired to ros2_control topics. EKF fusion + odom improvements are tracked separately. | 2026-04-02 | 2026-02-11 |
| 19 | Mechanical Integration: SO-101 Arms | <span style="color: green">Done</span> | Assembled SO-101 hardware and verified reach/clearance. Current AMR direction is one mounted SO-101 arm with power budget/fusing, harness routing, and strain relief tracked for that single-arm configuration. | 2026-05-07 | 2026-02-08 |
| 20 | Mechanical Assembly + Enclosure | <span style="color: green">Done</span> | Main AMR mechanical assembly completed, including SO-101 integration provisions. Follow-on hardware refinements now move into manipulation/perception work: wrist camera mount on SO-101, AMR depth sensor repositioning for better gripping, and any final cable routing/strain relief cleanup. | 2026-02-26 | 2026-04-17 |
| 21 | Mission Runtime & Sequencing (v1) | <span style="color: green">Done</span> | Promoted `amr_missions` from CLI-only use into a persistent `mission_server` with typed `MissionStatus`, a `/amr_missions/state` service, topic-command support, request validation, retries/timeouts/return-home handling, and direct integration into the one-shot navigation tmux workflow. | 2026-04-16 | 2026-04-28 |
| 22 | Firmware v2: Cascaded Control + Comparison | <span style="color: green">Done</span> | Cascaded-current-control investigation is complete for this AMR phase. Higher-accuracy current-sensor integration was brought up for protection/diagnostics, ROS current + ADC/zero topics were added, current polarity/sign were corrected, and the OC threshold was returned to `1500 mA`. Decision: keep production motion on the validated single-loop speed controller for now; use current sensing for protection/logging and revisit true cascaded current control only if traction or torque-control requirements justify it. | 2026-01-22 | 2026-05-03 |
| 23 | Safety Supervision & Health Monitoring | <span style="color: green">Done</span> | Implemented and validated the first production safety-supervision layer: STM comm/fault monitoring, safety intervention state, mission/Nav2 cancel path, guarded zero-velocity + STM disable flow, supervisor reset service, and repeatable operator recovery helper. Real motor-driver power-cut tests confirmed safe stop, blocked reset while STM faults were active, fault clear after power restore, manual STM re-enable, and successful mission continuation. Follow-up safety expansions can be tracked separately. | 2026-04-23 | 2026-05-03 |
| 24 | AMR Rewire + Known-Good Bring-Up | <span style="color: green">Done</span> | Rewired and brought the AMR back up on the Nano/Foxy baseline. Validated STM micro-ROS, `/amr_stm/*`, LiDAR `/scan`, wheel state/odom path, map/AMCL readiness, Nav2 lifecycle, mission server, safety supervisor, MCP launch path, and supervised named-place navigation. Defer STM-carrier proximity sensors, STM I2C IMU, and INA226 wiring until a custom carrier/control board; continue reliability cleanup through acceptance testing. | 2026-05-14 | 2026-05-23 |
| 25 | Agentic AMR Control Revalidation On Nano | <span style="color: green">Done</span> | Revalidated the guarded agentic control path on the existing Nano/Foxy baseline: robot-launch MCP restarted Jetson/dev-PC runtime, mission-control MCP readiness passed, and supervised `go_to hall` completed successfully through `/amr_missions/go_to` with STM fault mask clear. Live motion remains supervised and must use mission/safety/Nav2 paths, never raw `/cmd_vel` or direct wheel commands. | 2026-05-21 | 2026-05-23 |
| 26 | NUC Dev-PC Migration Baseline | <span style="color: green">Done</span> | NUC is validated as the live dev-PC replacement while Jetson Nano remains the hardware runtime: Docker/image/build checks passed, split NUC+Jetson bring-up works, the one-command navigation launcher works on tmux 3.4, and supervised mission-control runs succeeded for `hall`, `kitchen`, and `home` with STM fault mask clear. Extended repeatability/acceptance loops are intentionally deferred to the later hardware acceptance/reliability track. | 2026-05-30 | 2026-06-01 |
| 27 | Voice + Operator Command MVP | <span style="color: goldenrod">In Progress</span> | MCP-oriented voice/operator path is active: `hey jarvis` wake gating, Vosk command ASR and Faster Whisper push-to-talk support, Piper TTS through speaker MCP, conversation MCP planning, Qwen-backed local intent routing, pending confirmation handling, and read-only state-inspection tool execution for status/diagnostics. Regular Week-27 voice/operator work resumes after the immediate Orin runtime bring-up tasks. Motion remains gated through mission-control MCP readiness and explicit supervised confirmation. | 2026-04-30 | TBD |
| 28 | Manipulation Perception + Calibration | <span style="color: goldenrod">In Progress</span> | Use AMR-mounted depth camera for broad scene/object discovery and wrist camera for close-range grasp refinement. Low-bandwidth Orin-to-NUC preview topics are active for D455 color/depth and SO-101 wrist camera; raw/high-resolution streams should stay local to Orin for autonomy/training unless explicitly needed. Complete hand-eye calibration, base-to-arm extrinsics, camera-to-tool frames, AprilTag checks, RGB-D logging, and stale-frame rejection before grasp execution. | 2026-07-09 | TBD |
| 29 | Orin NX AMR Runtime Migration | <span style="color: goldenrod">In Progress</span> | Pulled forward as the pre-Week-27 focus while keeping NUC+Nano/Foxy as the validated fallback. Orin Docker profile created under `docker/orin`; image `amr/ros2-humble-orin:arm64` builds on Orin with ROS 2 Humble, Nav2, ros2_control, micro-ROS Agent, YDLidar driver, RealSense ROS driver, librealsense `2.57.7` using RSUSB, MoveIt2, `usb_cam`, and the Feetech Python dependency. AMR `ros_ws` builds inside the Orin Humble image after porting `amr_hardware` to Humble `ros2_control` lifecycle callbacks and updating controller-manager spawners; added `ydlidar_orin.yaml` and `orin_hardware.launch.py` for stable by-id paths. Supervised Orin bring-up has run LiDAR `/scan`, STM micro-ROS with `stm_link_ok`, active base controllers, `/amr/joint_states`, NUC RViz/dashboard/teleop over CycloneDDS, D455 low-bandwidth preview topics, and SO-101 wrist webcam preview. A tiny suspended Nav2 CLI goal reached the STM wheel-command path and completed; do not use suspended testing for localization quality because wheel odom moves while the room scan does not. RealSense raw streams should stay local to Orin unless explicitly needed; Wi-Fi viewing should use preview topics. Current checkpoint: keep mounted Orin power/USB/cooling stable, resume supervised floor hardware acceptance, and keep NUC+Nano/Foxy as fallback until Orin passes acceptance. | 2026-06-06 | TBD |
| 30 | Agentic AMR Operation Baseline On Orin | Planned | Port the revalidated agentic baseline to the Orin path after core devices and ROS contracts work there: skills, harnesses, shared ROS clients, state-inspection MCP, guarded mission-control MCP, guarded robot-launch MCP, and voice/conversation/speaker MCPs. Add software-only fixture tests for MCP/clients and harden confirmation-required command tools for `go_to_named_place`, `cancel_mission`, and supervised launch without bypassing mission/safety/Nav2. | 2026-06-13 | TBD |
| 31 | Supervised AMR Hardware Acceptance On Orin | Planned | Use the hardware acceptance harness on the real AMR with Orin as the target runtime once device bring-up is ready: idle, post-motion, localization readiness, mission success, safety recovery, fault decode, and repeated named-place routes. Compare against the known NUC+Nano/Foxy baseline and produce reports before enabling command MCPs for regular operation. | 2026-06-20 | TBD |
| 32 | Local LLM + Monitoring On Orin | Planned | Run a small local orchestration LLM and monitoring stack on Orin for operator dialogue, tool selection, status explanation, system health, and MCP calls. Keep deterministic parser paths for known commands. Enforce skills, permissions, confirmation, and AMR MCP safety gates before any motion-causing tool call. | 2026-06-27 | TBD |
| 33 | IMU + EKF State Estimation & Nav Validation | Planned | STM-carrier IMU wiring is deferred until a custom carrier/control board makes the sensor bus cleaner. If odometry improvement is needed before then, evaluate a Jetson-connected IMU or RealSense IMU as the candidate `/imu` source. Validate topic rate, timestamps, frame ID, orientation/gyro stability, covariance, mounting, and Docker/device access before enabling `robot_localization`. Then fuse wheel odom + IMU, define filtered-odom TF ownership, retest SLAM, AMCL, and Nav2, and include follow-up for motion-only AMCL/localization jumps seen during 2026-05-03 mission validation. | 2026-04-09 | TBD |
| 34 | URDF/MoveIt For Base + SO-101 | <span style="color: goldenrod">In Progress</span> | Single mounted SO-101 scaffold is active: optional AMR Xacro integration, upstream SO101 URDF/STL import, AMR mount wrapper, wrist-camera TF frames, MoveIt2 config, and encoder-captured named poses (`home`, `carry`, `look_from_height`, `ready_to_pick_up`). Next: measure mount offsets, calibrate wrist-camera extrinsics, and calibrate physical joint limits because some recorded safe poses exceed the starter URDF/MoveIt limits. | 2026-06-25 | TBD |
| 35 | Arm Control Bring-Up With MoveIt | <span style="color: goldenrod">In Progress</span> | `amr_so101_driver` now runs on Orin: `/so101_trajectory_bridge` serves `/so101_arm_controller/follow_joint_trajectory`, publishes `/so101/joint_states`, exposes `/so101/free_servos`, and `/amr_joint_state_merger` combines `/amr/joint_states` plus `/so101/joint_states` into `/joint_states`. Supervised MoveIt/teleop has moved wrist roll and then all six available SO-101 joints. Default real execution remains conservative; all-six execution requires explicit supervised bring-up until joint limits, torque margins, and calibration are finished. | 2026-07-02 | TBD |
| 36 | Classical Grasp Pipeline With MoveIt | Planned | Build the first measurable manipulation pipeline: perception -> object/grasp proposal -> MoveIt reachability/collision planning -> guarded execution -> retreat/recovery -> wrist-camera verification. Use this as the baseline before VLA/VLM action policies. | 2026-07-16 | TBD |
| 37 | Base + Arm Integration Demo | Planned | Navigate to a named pickup pose, align to workspace, run classical grasp, place at drop zone, report status, and recover from failed grasp or navigation errors. Maintain consistent base/arm/camera transforms and safety watchdogs. | 2026-07-23 | TBD |
| 38 | External MCP Integrations For Home Autonomy | Planned | Integrate useful non-motion MCPs with the Orin LLM agent: Home Assistant for home state/devices, system monitoring for Orin health, calendar/tasks/reminders, and recipe/meal-planning or shopping-list MCPs. Keep robot motion behind AMR-owned MCP safety tools only. | 2026-07-30 | TBD |
| 39 | AMR Perception MCPs + Household Inspection | Planned | Add custom AMR perception MCP tools for camera inspection workflows such as workspace/fridge/counter scan, object list proposals, stale-frame checks, and operator confirmation. Connect outputs to recipe/task/home MCPs where useful, but do not let perception directly command actuators. | 2026-08-06 | TBD |
| 40 | VLM/VLA Trials For Manipulation | Planned | Profile VLM/VLA models on Orin and/or external GPU fallback. Use them for object discovery, grasp/action proposals, and scene inspection while MoveIt and guarded controllers remain responsible for planning/execution. Compare against the classical grasp baseline. | 2026-08-13 | TBD |
| 41 | Agentic Home Autonomy Demo | Planned | Demonstrate an agentic workflow that combines AMR operation, LLM orchestration, MCP integrations, and guarded manipulation: inspect a scene, suggest an action or meal/task, schedule/remind through calendar/task MCPs, optionally navigate or manipulate through AMR-owned safety-gated MCP tools, and log all decisions/confirmations. | 2026-08-20 | TBD |
| 42 | Productization Scope Freeze | Planned | Freeze one product-like showcase workflow: fixed-place indoor delivery/navigation with human load/unload. Define what is in scope, what is explicitly out of scope, known limitations, demo route, operator workflow, and acceptance criteria. Do not add broad autonomy features unless they improve this workflow. | 2026-08-27 | TBD |
| 43 | Repo Productization Baseline | Planned | Add top-level `README.md`, `Makefile`, `.env.example`, structured docs index, and cleanup plan for generated artifacts/placeholders. Convert `useful commands.md` into documented one-command workflows. Separate dev, ops, diagnostics, calibration, and hardware-acceptance scripts. Add the agentic robotics documentation baseline: agent roles, tool permissions, MCP wrapper plan, repo-local skills plan, and interaction examples. | 2026-09-03 | TBD |
| 44 | Automated Test Foundation | Planned | Add unit tests for `amr_voice` parser, mission config loading, mission request validation, and safety-supervisor health evaluation. Add `colcon test` workflow locally. Start with software-only tests that do not need hardware. Add the first agent harness scenarios for unsafe motion refusal, localization-not-ready denial, safety-fault denial, and manipulator plan-before-execute behavior. | 2026-09-10 | TBD |
| 45 | CI/CD Foundation | Planned | Add GitHub Actions or equivalent CI that runs formatting/lint checks, Python unit tests, ROS package build, and selected launch/config smoke checks on every PR/branch. CI should block regressions before hardware testing; CD remains limited to tagged Docker/image/release artifacts until deployment is safer. | 2026-09-17 | TBD |
| 46 | Hardware Acceptance & Reliability Report | Planned | Turn baseline probes into a repeatable hardware acceptance suite: idle, motion, post-motion, localization readiness, fault decode, safety recovery, and mission success criteria. Run 20+ repeated delivery/navigation missions and summarize success rate, faults, localization jumps, recovery time, and known limitations. | 2026-09-24 | TBD |
| 47 | Custom Carrier PCB Development | Planned | Design and validate the custom carrier/control-board path before adding extra STM-side sensors: connectorized STM/driver/sensor wiring, clean 5 V/3.3 V distribution, I2C routing, level shifting/protection, grounds, test points, fusing, and harness labels. Start with a carrier/backplane around the current Nucleo/Cytron/buck architecture before considering deeper motor-driver or MCU integration. | TBD | TBD |
| 48 | Post-PCB Proximity + Battery Level Integration | Planned | After the custom carrier/control board exists, integrate proximity sensors and INA226-style pack telemetry cleanly: proximity trigger/echo routing and filtering, shared I2C battery monitor wiring, external shunt placement after the main fuse and before branch distribution, STM firmware drivers, `/amr_stm/*` telemetry topics, docs, harness contracts, and acceptance checks. | TBD | TBD |
| 49 | Product-Grade Demo Package | Planned | Produce a polished productization case study: operator demo, deployment guide, safety case, acceptance-test report, architecture diagram, BOM/cost estimate, release tag, and demo video. Present the AMR as a product-grade indoor delivery platform showcase, not a general-purpose robot claim. | 2026-10-01 | TBD |
...
## Status Summary
- Overall: SLAM + localization + navigation pipeline is working on real hardware (slam_toolbox map -> saved map -> AMCL -> Nav2 goals). The current validated live fallback remains NUC-as-dev-PC plus Jetson Nano-as-hardware-runtime on ROS 2 Foxy: the NUC runs `amr_devpc`, RViz/Nav2/mission/safety/agent tooling; the Jetson runs `amr_foxy`, micro-ROS, LiDAR, ros2_control, and STM link watchdog. The Orin NX/Humble migration path now has Docker, hardware launch, NUC RViz/dashboard/teleop helpers, LiDAR, STM, controllers, RViz goal delivery, camera previews, SO-101 MoveIt bridge execution, and a suspended Nav2 command-path smoke test. SO-101 integration and an initial ACT manipulation demo are validated; low-level control/odometry calibration is still in active tuning.
- Progress: 26/49 weeks complete (~53%). NUC migration is closed as a baseline milestone; extended repeatability tests are deferred to the later hardware acceptance/reliability track. The active near-term focus has moved from Orin software bring-up to supervised integration cleanup: keep the mounted Orin running with stable power/USB/cooling, validate floor operation, and calibrate SO-101 joint limits and camera extrinsics before grasp execution.
- Recent:
  - Validated the NUC as the live dev-PC replacement for the existing split runtime. Docker, amd64 AMR images, software-only `colcon build/test`, map artifacts, Jetson SSH aliases, UDP/DDS firewall rules, and NUC-to-Jetson reduced source sync are in place.
  - Completed supervised NUC+Jetson mission validation on `my_new_map`: `go_to hall`, then `go_to kitchen`, then `go_to home` all succeeded through guarded mission-control and `mission_server`. Safety stayed monitor-only/healthy, STM `fault_mask` stayed `0`, comm stayed `stm_link_ok`, and odom returned to zero after arrival.
  - Fixed and validated the NUC one-command launcher path: `open_amr_devpc_navigation.sh` now uses tmux 3.4-compatible pane sizing, creates `amr_devpc_nav`, starts Jetson `amr_foxy`, resets STM, brings up Nav2/RViz/mission/safety panes, and passes guarded dry-run mission readiness after initial pose.
  - Validated the AMR MCP surface for future OpenClaw integration: source compile, agent harness checks, host-safe MCP smoke tests, ROS-attached state/mission MCP smoke tests inside `amr_devpc`, launch preflight, live read-only inspection, and mission-control dry-run readiness all passed without sending motion.
  ...
  - Completed the AMR rewire/known-good bring-up loop on the Nano/Foxy runtime: MCP robot launch restarted the Jetson hardware stack and dev-PC Nav2 session with `my_new_map`, STM reset over ST-LINK completed, `/scan` and `/amr_stm/wheel_state` were live, AMCL/Nav2 lifecycle readiness passed after initial pose, and mission services were responsive.
  - Revalidated the supervised agentic mission path end-to-end: MCP mission-control readiness passed for `hall`, `go_to_named_place("hall")` started through `/amr_missions/go_to`, the robot reached `hall`, both wheel encoder paths responded during motion, and STM `fault_mask` stayed `0`.
  - Added a launcher hardening option to run the MCP/dev-PC navigation stack with RViz disabled (`AMR_START_RVIZ=false`, MCP `start_rviz=false`) after RViz load was observed to make lifecycle service readiness flaky.
  - Debugged the recent hardware bring-up faults: left-stall evidence pointed to wiring/encoder-response issues during earlier tests, a wiring issue was corrected, and a transient right current-sensor rail/OC fault was identified as a sensor wiring/calibration follow-up rather than a Nav2/MCP command-path issue.
...
- Current Focus (next sprint):
  - Keep the Jetson Orin NX mounted/powered on the AMR with stable cable routing, cooling, USB devices, and repeatable restart behavior.
  - Continue Orin floor bring-up only under supervision, with the validated NUC+Nano/Foxy path kept intact as fallback.
  - Keep the validated NUC+Nano/Foxy path intact as the fallback; do not silently replace `docker/foxy`, Nano launch assumptions, or the known-good mission workflow.
  - Use the Orin-specific Docker/runtime profile (`docker/orin`, image `amr/ros2-humble-orin:arm64`) as the Humble/JetPack 6 base; keep the Orin workspace synchronized from the NUC before rebuilds.
  - Bring up Orin hardware incrementally after power loss: USB/device IDs, STM32 micro-ROS, `/amr_stm/*` topics, LiDAR `/scan`, RealSense preview streams, wrist webcam preview, SO-101 serial path, GPU/container runtime, thermals, network, and device permissions. Initial bench/suspended checks passed; the next meaningful validation is floor hardware acceptance and manipulation calibration.
  - Port or shim Foxy-to-Humble differences, especially `ros2_control`, `controller_manager`, launch files, Nav2 parameters, and micro-ROS agent compatibility.
  - Prefer the validated one-command NUC launch over manual container bringup for fallback operations; keep the manual flow only as a debugging fallback.
  - Keep the current hardware work limited to reliability cleanup: cable strain relief, current-sensor wiring/calibration checks, USB/power stability, and sensor mounting that supports the existing base. Defer STM-carrier proximity sensors, STM I2C IMU, and INA226 battery monitor wiring until a custom carrier/control board.
  - Harden the MCP launch/mission workflow around the validated NUC path: use mission-control readiness as the motion gate, preserve explicit supervised confirmation, and add fixture/software tests for the MCP clients.
  - Keep live agentic motion supervised and routed through mission/safety/Nav2 only. Do not use raw `/cmd_vel`, direct wheel commands, fault clear, STM re-enable, or launch changes without explicit supervised confirmation.
  - Defer broad repeatability loops until the hardware acceptance/reliability track; use only targeted checks needed to prove each Orin interface during bring-up.
  - Treat IMU/EKF as optional after bring-up. If needed before the custom board, evaluate a Jetson-connected IMU or RealSense IMU source, validate `/imu` quality first, and only then configure `robot_localization`.
...
- Next Focus:
  - Continue Orin NX AMR runtime migration on the mounted/powered robot while keeping NUC+Nano/Foxy as the known-good fallback until Orin passes hardware acceptance.
  - Agentic AMR operation hardening on the Orin-capable base through voice/text/LLM -> skill -> MCP -> shared ROS client -> mission/safety/Nav2.
  - MoveIt-first manipulator bring-up after the AMR base is stable. Current first slice now includes the upstream-backed single SO-101 URDF/TF scaffold, wrist-webcam frame, MoveIt2 config, conservative `amr_so101_driver` trajectory bridge, all-six supervised teleop, free-servo service, and encoder-captured named poses. Next manipulator action is physical joint-limit calibration before trusting home/carry/look/pick poses in MoveIt.
  - Orin LLM/MCP integrations after the Orin AMR runtime and safety boundaries are proven.
  - Supervised repeatability and hardware acceptance on Orin, with comparison against the known-good NUC+Nano/Foxy split.
- Timeline: Flexible (now a 49-week plan with extensions). Prioritize Orin runtime migration first while preserving the NUC+Nano/Foxy fallback, supervised hardware acceptance second, MoveIt manipulation third, and broader Orin-based autonomous integrations after the core robot is reliable. Custom PCB is now the preferred place to integrate proximity sensors, STM-side IMU, and INA226 battery monitoring cleanly.
...
- Keep any second-arm or dual-arm manipulation as a future capability until navigation, safety, logging, and the single-arm workflow are repeatable.
...
## Project Log
- 2026-07-07: Updated the Orin/SO-101 checkpoint after live AMR operation. The Orin AMR hardware container is the current Humble bring-up path with LiDAR, STM micro-ROS, controllers, `/amr/joint_states`, D455 low-bandwidth preview topics, and the SO-101 wrist webcam preview available to the NUC over CycloneDDS. Raw/high-resolution camera topics stay local to Orin unless explicitly needed.
- 2026-07-07: Validated the SO-101 MoveIt bridge far beyond the original wrist-roll-only slice under explicit supervision. The bridge now supports `/so101_arm_controller/follow_joint_trajectory`, `/so101/joint_states`, `/so101/free_servos`, merged global `/joint_states`, all-six joint teleop helpers, named-pose helpers, and encoder-captured `home`/`carry`, `look_from_height`, and `ready_to_pick_up` poses. Remaining blocker is physical joint-limit calibration: recorded safe poses expose starter URDF/MoveIt limit mismatches, so limits must be measured and updated with margins before autonomous grasp execution.
- 2026-06-08: Updated the persistent `the-amr-guy` context, manipulator architecture, ROS topic ownership, and project tracker for the SO-101 MoveIt bridge. Current arm topic contract is `/amr/joint_states` for base wheels, `/so101/joint_states` for the arm, merged `/joint_states` for RViz/MoveIt, and `/so101_arm_controller/follow_joint_trajectory` for MoveIt execution. The driver defaults to fake hardware and real execution remains wrist-roll-only until Orin validation and shoulder/elbow characterization are complete.
- 2026-06-08: Added the first source-level SO-101 MoveIt execution bridge in `amr_so101_driver` with a conservative `FollowJointTrajectory` action server, Feetech STS3215 serial backend, fake hardware mode, wrist-roll-only execution gate, and joint-state merger for AMR base + arm compatibility. Local Python syntax checks passed; full `colcon build` and supervised hardware validation are pending until the Orin is powered again.
- 2026-06-04: Added the revised Jetson Orin NX enclosure print as an explicit mechanical-integration backlog item. The current enclosure design was insufficient, so the next Orin action is to print one more revised enclosure, install it, power the Orin from the AMR, and only then resume supervised floor bring-up/hardware acceptance.
- 2026-06-02: Started the single mounted SO-101 manipulator stack. Vendored the upstream TheRobotStudio/SO-ARM100 SO101 URDFs, Apache-2.0 attribution, and 13 STL assets; added a prefixed Xacro wrapper with AMR top-plate mount guess, `so101_tool0`, and wrist-webcam frames. Added a view launch file and planning-only starter named poses. The scaffold is for RViz/MoveIt bring-up only; no arm driver, gripper actuation, trajectory controller, or hardware execution path is enabled.
- 2026-06-01: Closed Week 26 as the NUC dev-PC migration baseline. The validated NUC+Nano/Foxy split remains the fallback, while extended route repeatability loops are deferred to the later hardware acceptance/reliability track. Pulled Jetson Orin NX runtime migration forward as the immediate pre-Week-27 focus: create a separate Orin/JetPack 6 runtime profile, likely ROS 2 Humble, build the ROS stack there, validate STM32 micro-ROS/LiDAR/RealSense/GPU/USB/network/device access incrementally, and only promote Orin after supervised hardware acceptance.
- 2026-06-01: Advanced the Orin NX/Humble migration to a paused bench-ready checkpoint. Added the Orin Docker runtime, NUC RViz image, CycloneDDS peer setup, UDP check, Orin hardware launch, YDLidar Orin config, NUC dashboard, NUC RViz launcher, and Orin teleop helper. Verified the Orin hardware container with LiDAR, STM micro-ROS, ros2_control, joint states, wheel state, RViz map/scan/model visualization, Nav2 lifecycle, and RViz goal action clients. A tiny supervised suspended Nav2 goal reached the STM wheel-command path and completed; suspended localization behavior is not meaningful because odom changes without room-relative scan motion. RealSense color is usable in RViz, while depth remains diagnostic-only due USB/control-transfer and Right MIPI warnings. Next action is mechanical/electrical, not software: print/install the Orin enclosure, power it from the AMR, then resume supervised floor acceptance.
- 2026-05-30: Validated the AMR MCP surface ahead of OpenClaw integration. Host-safe MCP smoke tests passed for conversation, perception inspection, robot launch, speaker, and voice interface. ROS-attached smoke tests passed inside `amr_devpc` for state inspection and mission control. Live read-only checks passed for robot health, safety, localization, navigation, mission state, named places, and STM diagnostics; guarded mission-control dry-run readiness for `hall` passed; robot-launch preflight/dry-run passed for `my_new_map`. Captured the checkpoint in `docs/agentic/mcp_validation_2026-05-30.md`.
- 2026-05-30: Extended the NUC live validation with a supervised two-leg route. Guarded readiness passed for `kitchen` and `home`; with operator supervision confirmed, `go_to kitchen` succeeded with `reached 'kitchen'`, then a fresh readiness check passed and `go_to home` succeeded with `reached 'home'`. Safety remained monitor-only/healthy, STM `fault_mask` stayed `0`, comm stayed `stm_link_ok`, and odom returned to zero after arrival.
- 2026-05-30: Fixed and validated the NUC one-command navigation launcher. The NUC host uses tmux 3.4, where `split-window -p <percent>` fails with `size missing`; updated `scripts/open_amr_devpc_navigation.sh` to use `split-window -l <percent>%`. Non-attaching one-command launch now creates `amr_devpc_nav`, starts Jetson `amr_foxy`, resets STM, starts NUC `amr_devpc`, brings up Nav2/RViz/mission/safety panes, accepts the known `home` initial pose, and passes guarded dry-run readiness for `hall` with STM `fault_mask=0`.
- 2026-05-30: Validated the NUC as a live dev-PC replacement against the Jetson Nano hardware runtime. After a clean split relaunch, `my_new_map` loaded, RViz initial pose was accepted, AMCL/Nav2 readiness passed, mission/safety services were healthy, and the guarded mission-control readiness check for `hall` had no blockers. With explicit supervised operator confirmation, `go_to hall` was sent through `mission_server` and succeeded with final detail `reached 'hall'`; STM `fault_mask` stayed `0`, comm stayed `stm_link_ok`, safety stayed monitor-only/healthy, and odom returned to zero. Manual container bringup was the initial validated fallback before the tmux launcher compatibility fix.
- 2026-05-30: Cloned `KartikMehta-1/AMR-development` onto a NUC-class Ubuntu 24.04 x86_64 host at `/home/ubuntu/agent/repos/AMR-development` and captured the migration context in `docs/agentic/nuc_migration_context.md`. Docker Engine 29.1.3, Compose 2.40.3, and Buildx 0.30.1 were installed and `docker run --rm hello-world` passed from the operator's interactive shell. The current validated robot workflow remains the Nano/Foxy split until the NUC path is validated. Treat the NUC first as a dev-PC replacement, then separately evaluate whether it should become the on-robot computer with a dedicated hardware-facing runtime profile.
- 2026-05-23: Completed the Nano/Foxy AMR rewire and agentic control revalidation milestone. The robot was relaunched through the MCP robot-launch server on `my_new_map`, mission-control MCP readiness passed, and supervised `go_to hall` succeeded through the mission server with both wheel encoder paths responding and STM `fault_mask=0`. Added the RViz-disabled launch option (`AMR_START_RVIZ=false`, MCP `start_rviz=false`) after RViz load was observed to make Nav2 lifecycle service readiness flaky. Next focus is repeatability/hardware acceptance, current-sensor wiring/calibration follow-up, and then Orin migration.
- 2026-05-23: Sorted the canonical week table by status so completed work appears first, active work follows, and planned work appears in intended execution order. Moved proximity sensing and INA226 battery-level integration behind custom carrier/control PCB development, combining the old immediate proximity hardware/driver slots into a post-PCB integration task.
- 2026-05-23: Reprioritized the near-term plan after deciding not to force proximity sensors, STM-side IMU, or INA226 onto the current STM carrier wiring. These sensors are deferred to a future custom carrier/control board unless a simple Jetson-connected IMU is needed for odometry experiments. Immediate focus is now to rewire the AMR back to the known-good base configuration, bring up the existing Nano/Foxy stack, revalidate the agentic control path on that baseline, and then proceed with Orin NX runtime migration.
